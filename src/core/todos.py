# allowing agents to work on long tasks, by giving them a todos feature -> splitting long task into smaller sub tasks
...
def write_todos(todos):
    """Replace the whole list. Exactly one task may be in progress"""
    active = [t for t in todos if t["status"] == "in_progress"]
    if len(active) > 1:
        return f"Error: {len(active)} tasks are in_progress. Only one may be."

    #empties the original TODOS list and fills it with the items from todos
    TODOS[:] = todos
